...
def test_cache_dir():
    ...
    """
    original_cache_dir = package_repo_scraper.CACHE_DIR
    test_cache_tar = os.path.join(os.path.dirname(__file__), "test_cache.tar")
    ...
    if os.path.exists(test_cache_tar):
        with tarfile.open(test_cache_tar, "r") as tar:
            tar.extractall()
        print(f"Extracted test cache to {test_cache_dir}")
    else:
        print(f"Warning: test_cache.tar not found at {test_cache_tar}")

    # Patch the CACHE_DIR variable
